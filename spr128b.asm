...
proc16 gravity_balls
    ...
    updatesprite:
        ...
        bne .moveright
        jsr bounce_balls.move_left
        bra +
    
    .moveright:
        jsr bounce_balls.move_right
    +
        setas
        lda tdir
        sta ball_dir,x
...
proc16 move_balls_diagonally
    ...
    setxpos:
        ...
        cmp #(max_sprite_ypos)
        bcc +
        lda #(min_sprite_ypos-sprite_height)
        bra setypos
    +
        cmp #(min_sprite_ypos-sprite_height)
        bcs setypos
        ; wrap around
        lda #(max_sprite_ypos)
    setypos:   
...
proc16 move_balls_sideways
    ...
    updatesprite:
        ...
        cmp #(max_sprite_xpos)
        bcc +
        lda #(min_sprite_xpos-sprite_width)
        bra setxpos
    +
        cmp #(min_sprite_xpos-sprite_width)
        bcs setxpos
...
proc8 init_ball_sprites
    ...
    sprstartpos:
        for spry = 0 to 7
            for sprx = 0 to 15
                word sprite_screen_offset+(sprx*20) , sprite_screen_offset+24+(spry*24)
            next
        next
endproc
